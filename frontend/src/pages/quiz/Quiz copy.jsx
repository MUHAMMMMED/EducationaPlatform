...
const QuizComponent = () => {
  ...
  useEffect(() => {
    if (quizData && time >= quizData.exam.time_to_answer) {
      ...
      setResult(true);
      submitExam();
    } else if (quizData && quizData.exam.time_to_answer - time === 5) {
  ...
  <Result>
    ...
    <div className="score"> <span  className='span_icon'><GrScorecard /></span>You scored {score} out of {questions.length}</div>
    <div className="time"> <span className='span_icon'><GrDocumentTime /></span>Total Time Taken: {formatTime(time)}</div>
