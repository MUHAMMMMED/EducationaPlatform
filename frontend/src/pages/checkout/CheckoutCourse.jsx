...
import FreeCheckoutCourse from './FreeCheckoutCourse';
...
const CheckoutCourse = ({ Id, name, price, Img }) => {
    ...
    const handleApplyCoupon = async () => {
        if (couponCode.trim() !== '') {
            try {
                const response = await AxiosInstance.post(`${Config.baseURL}/payments/Check-CouponCode-Courses/`, { id: Id, couponCode }, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
                        'Content-Type': 'application/json'
                    }
