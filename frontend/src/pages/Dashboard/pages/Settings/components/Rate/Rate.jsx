    ...
    <>
      <div className="CourseCard">
        <div className="Course_card_content">
          <div className="Course_card_info">
            <div style={{ float: 'left', width: '65px' }}>
              <span className="onLine-icon"><MdOutlineStarRate /></span>
            </div>
        ...
        {date && date.rate && date.rate.slice(0, displayCount).map((item) => {
          console.log('Rate number:', item.rate_number, 'Parsed:', parseInt(item.rate_number));
          ...
          return (
            <div className="CourseCard" key={item.id} style={{ width: '100%', marginLeft: '0', marginTop: '1', border: '1px solid #ddd;' }}>
              <div className="Course_card_content">
                <div className="Course_card_info">
                  <div style={{ float: 'left', width: '65px' }}>
                    <span className="onLine-icon"><FaRegUser /></span>
                  </div>
                  <div style={{ float: 'left' }}>
                    <p className="Course_card_title">
                      <p style={{ marginLeft: '10px' }}>
                        {item?.student?.user_full_name} -
                        {[...Array(isNaN(parseInt(item.rate_number)) || parseInt(item.rate_number) < 0 ? 0 : parseInt(item.rate_number))].map((_, index) => (
              ...
              <div className="Course_card_content">
                <div className="Course_card_info">
                  <div style={{ float: 'left', width: '65px' }}>
                    <span className="onLine-icon"><FaQuoteRight /></span>
                  </div>
                  <div style={{ float: 'left' }}>
                    <p className="Course_card_title" style={{ textAlign: 'center' }}>
                      <FaQuoteLeft />-<span> {item.message}</span> - <FaQuoteRight />
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="Course_card_info" style={{ alignItems: "center" }}>
          <div className="Course_cardicon" style={{ width: '60%' }}>
            {date && date.rate && date.rate.length > displayCount && (
              <button className="Open_button" onClick={handleDisplayMore}>Load More</button>
            )}
          </div>
        </div>
      </div>
