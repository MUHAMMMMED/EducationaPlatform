...
export default function LiveCoursefaqsFormCreate({ liveCoursesId, fetchCourse }) {
  const [showModalfaqsCreate, setShowModalfaqsCreate] = useState(false);
  const [formData, setFormData] = useState({
    course: liveCoursesId,
    question: '',
    answer: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await AxiosInstance.post(`${Config.baseURL}/LiveCourses/live-course-faqs/`, formData);
      setShowModalfaqsCreate(false);
      setFormData({ course: liveCoursesId, question: '', answer: '' });
