...
export default function PromoCodes({ Id }) {
    ...
    const fetchCouponCodes = async () => {
        try {
            ...
            const response = await AxiosInstance.get(`${Config.baseURL}/LiveCourses/live-course-coupon-codes/${Id}/`);
    ...
    const handleDelete = async (id) => {
        try {
            ...
            await AxiosInstance.delete(`${Config.baseURL}/LiveCourses/live-course-coupon-codes-delete/${id}/`);
        ...
        <>
            <div className='section_PromoCodes'>
                ...
                <div class="head-flex-container">
                    <div>Promo Code </div>
                    <div>  <CouponCodeForm Id={Id} fetchCouponCodes={fetchCouponCodes} />
                ...
                {couponCodes.map(code => (
                    <div class="Promo_card" key={code.id}><samp className='span_x' onClick={() => handleDelete(code.id)}>X</samp>
                        <h4 >{code.Code}</h4>
                        <div class="card-Main"> <span>{code.Enroll}  </span> <p id='sale' >{code.discount} %</p>
                        </div>
