...
import QuestionForm from '../../../../../../../DashboardQuizDetails/components/QuestionBank/components/AddQuestion/QuestionForm/QuestionForm';
...
import CourseAddToQuiz from '../CourseAddToQuiz/CourseAddToQuiz';
    ...
    <div className="CourseCard" style={{ border: '0px solid #58a58f', boxShadow: 'none' }}>
      <div className="Course_card_content">
        ...
        <div className="Course_cardicon">
          {creator && <QuestionForm data={data} creator={creator} fetchQuestions={fetchQuestions} />}
        </div></div>
      {data.questions && data.questions.map((item, index) => (
        <div className="CourseCard" key={index} style={{ width: '100%', marginLeft: '0', marginTop: '1', border: '1px solid #ddd;' }}>
          <div className="Course_card_content">
            ...
            {item && item.id && <QuestionUpdate Item={item} data={data} fetchQuestions={fetchQuestions} />}
          </div>
          <ul className="wrong-answers-list">
            <li >
              {item.question_content && (<p className="question-content">  {item.question_content}</p>)}
              {item.question_image && (
                <img style={{ width: "100%", border: '1px solid #d1ae20 ', borderRadius: ' 10px' }} src={`${Config.mediaURL}${item.question_image}`} />)}
          ...
          <div className="CourseCard" style={{ width: '100%', marginLeft: '0', marginTop: '1', border: '1px solid #ddd;' }}>
            <div className="Course_card_content">
              ...
              <div className="Course_cardicon">
                {item.id && exams && <CourseAddToQuiz questionId={item.id} exams={exams} fetchQuiz={fetchQuestions} />}
              </div></div>
